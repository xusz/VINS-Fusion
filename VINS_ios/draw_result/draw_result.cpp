                            ...
                            Matrix3f R_latest)
{
    ...
    for (unsigned int i = 0; i < point_cloud.size(); i++)
    {
        ...
        cv::Point2f pts;
        pts.x = FOCAL_LENGTH_X * Pc.x() / Pc.z() + PX;
        pts.y = FOCAL_LENGTH_Y * Pc.y() / Pc.z() + PY;
...
//    if (point_inlier.size() > 28)
    ...
    if (locationX != locationX_p || locationY != locationY_p)
    {
        ...
        for (unsigned int i = 0; i < Grounds.size(); i++)
        {
            ...
            box_center_xy.x() = FOCAL_LENGTH_X * Pc.x() / Pc.z() + PX;
            box_center_xy.y() = FOCAL_LENGTH_Y * Pc.y() / Pc.z() + PY;
        ...
        if (dis_min < 120 && Grounds.size() > 0 && Grounds[dis_min_idx].moveflag && finger_state == 1)
        {
            ...
            float rotation_angle = atan2(sqrt((cy-yy)*(cy-yy) + (xx-cx)*(xx-cx)), FOCAL_LENGTH_X);
    ...
    if (longPressFlag)
    {
        ...
        if (dis_min < 120 && Grounds.size() > 0)
        {
            ...
            pts1.x = FOCAL_LENGTH_X * Pc.x() / Pc.z() + PX;
            pts1.y = FOCAL_LENGTH_Y * Pc.y() / Pc.z() + PY;
    ...
    if (tapFlag && point_inlier.size() > 20)
    {
        ...
        {
            Pc = (R_latest * RIC).transpose()* (Grounds[i].center - 1.0*P_latest - R_latest * Vector3f(TIC_X,TIC_Y,TIC_Z));
            box_center_his.x() = FOCAL_LENGTH_X * Pc.x() / Pc.z() + PX;
            box_center_his.y() = FOCAL_LENGTH_Y * Pc.y() / Pc.z() + PY;
