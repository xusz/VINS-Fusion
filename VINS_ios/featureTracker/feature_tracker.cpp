    ...
    */
    ...
    if (prev_pts.size() > 0)
    {
        ...
        if (hasPrediction)
        {
            ...
            cv::calcOpticalFlowPyrLK(prev_img, cur_img, prev_pts, cur_pts, status, err, cv::Size(21, 21), 3);
        // reverse check
        // 反向的光流跟踪，通过上一帧跟踪到的点，反向跟踪回去，差异大的点剔除。 默认打开->bot.yaml
        if (FLOW_BACK)
        {
    ...
    if (1)
    {
        //rejectWithF();  // TODO: 为什么要注掉，是因为使用了反向光流？？？
...
void FeatureTracker::rejectWithF()
{
    if (cur_pts.size() >= 8)
    {
        ...
        for (unsigned int i = 0; i < cur_pts.size(); i++)
        {
            ...
            // liftProjective（）调用图像像素投影函数，将图像特征点的坐标映射到空间坐标，里面涉及处理畸变的过程；
            // 最后得到两组特征点的位置，通过这俩组特征点得到基础矩阵，通过基础矩阵剔除一些不好的点。
            //
            // 对于CATA（卡特鱼眼相机）才是将像素坐标投影到单位圆内，这里涉及了鱼眼相机模型，
            // 而对于PINHOLE（针孔相机）是将像素坐标直接转换到归一化平面（z=1）并采用逆畸变模型(k1,k2,p1,p2)去畸变
...
vector<cv::Point2f> FeatureTracker::undistortedPts(vector<cv::Point2f> &pts, camodocal::CameraPtr cam)
{
    ...
    for (unsigned int i = 0; i < pts.size(); i++)
    {
        ...
        Eigen::Vector3d b;
        cam->liftProjective(a, b);   // TODO:
        // 投影点3D -> 非畸变2D，b(x,y,z)为P点在相机归一化平面的投影，/z为转二维齐次坐标
