...
PinholeCamera::liftProjective(const Eigen::Vector2d& p, Eigen::Vector3d& P) const
{
    ...
    mx_d = m_inv_K11 * p(0) + m_inv_K13;   // K11 - > fx K13->cx p为像素在RAW图像中的坐标点
    my_d = m_inv_K22 * p(1) + m_inv_K23;   // K2 ->fy K23->cy
...
{
    double k1 = mParameters.k1();  // 径向畸变k1
    double k2 = mParameters.k2();  // 径向畸变k2
    double p1 = mParameters.p1();  // 径向畸变p1
    double p2 = mParameters.p2();  // 径向畸变p2
    ...
    rad_dist_u = k1 * rho2_u + k2 * rho2_u * rho2_u;  // 径向畸变因子
    d_u << p_u(0) * rad_dist_u + 2.0 * p1 * mxy_u + p2 * (rho2_u + 2.0 * mx2_u),  // 切向畸变纠正
           p_u(1) * rad_dist_u + 2.0 * p2 * mxy_u + p1 * (rho2_u + 2.0 * my2_u);  // 切向畸变纠正
}
