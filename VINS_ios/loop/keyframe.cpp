                           ...
                           vector<cv::Point2f> &measurements_old_norm)
{
    if (measurements_old.size() >= 8)
    {
        ...
        {
            cv::Point2f norm_pt;
            norm_pt.x = (measurements_old[i].x - PX)/FOCAL_LENGTH_X;
            norm_pt.y = (measurements_old[i].y - PY)/FOCAL_LENGTH_Y;
                                     ...
                                     std::vector<cv::Point2f> &measurements_predict)
{
    ...
    for (int i = 0; i < (int)point_clouds.size(); i++)
    {
        ...
        Vector2d point_uv;
        point_uv.x() = FOCAL_LENGTH_X * pts_predict.x() + PX;
        point_uv.y() = FOCAL_LENGTH_Y * pts_predict.y() + PY;
 ...
 * 将空间的3D点构建当前关键帧的特征点
 */
void KeyFrame::buildKeyFrameFeatures(Estimator &vins)
{
    for (auto &it_per_id : vins.f_manager.feature)
    {
        ...
        if (it_per_id.start_frame + frame_size >= WINDOW_SIZE - 1 && frame_size >= 3)
        {
            ...
            Vector2d point_uv;
            point_uv.x() = FOCAL_LENGTH_X * point.x()/point.z() + PX;
            point_uv.y() = FOCAL_LENGTH_Y * point.y()/point.z() + PY;
